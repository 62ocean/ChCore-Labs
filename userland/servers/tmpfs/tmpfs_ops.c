...
int fs_creat(const char *path)
{
	...
	/* LAB 5 TODO BEGIN */
	if(tfs_namex(&dirat, &leaf, 1) || tfs_creat(dirat, leaf, strlen(leaf))){
		return -1;
	}
...
int tmpfs_unlink(const char *path, int flags)
{
	...
	/* LAB 5 TODO BEGIN */
	err = 0;
	if (tfs_namex(&dirat, &leaf, 1) || tfs_remove(dirat, leaf, strlen(leaf))) {
		return -1;
	}
	/* LAB 5 TODO END */
	return err;
...
int tmpfs_mkdir(const char *path, mode_t mode)
{
	...
	/* LAB 5 TODO BEGIN */
	err = 0;
	if (tfs_namex(&dirat, &leaf, 1) || tfs_mkdir(dirat, leaf, strlen(leaf))) {
		return -1;
	}
	/* LAB 5 TODO END */
	return err;
